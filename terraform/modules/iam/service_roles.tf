...
# Vault IAM Role for External Secrets Operator
resource "aws_iam_role" "vault_external_secrets" {
  name = "${var.project_prefix}-${var.environment}-vault-external-secrets-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      ...
      Condition = {
        StringEquals = {
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:sub" = "system:serviceaccount:external-secrets-system:external-secrets-sa"
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:aud" = "sts.amazonaws.com"
        }
      }
    }]
...
resource "aws_iam_role_policy_attachment" "vault_external_secrets" {
  role       = aws_iam_role.vault_external_secrets.name
  policy_arn = aws_iam_policy.vault_external_secrets.arn
}

# Vault IAM Role for Production Namespace
resource "aws_iam_role" "vault_production" {
  name = "${var.project_prefix}-${var.environment}-vault-production-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      ...
      Condition = {
        StringEquals = {
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:sub" = "system:serviceaccount:production:production-vault-sa"
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:aud" = "sts.amazonaws.com"
        }
      }
    }]
...
resource "aws_iam_role_policy_attachment" "vault_production" {
  role       = aws_iam_role.vault_production.name
  policy_arn = aws_iam_policy.vault_external_secrets.arn
}

# Vault IAM Role for Monitoring Namespace
resource "aws_iam_role" "vault_monitoring" {
  name = "${var.project_prefix}-${var.environment}-vault-monitoring-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      ...
      Condition = {
        StringEquals = {
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:sub" = "system:serviceaccount:monitoring:monitoring-vault-sa"
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:aud" = "sts.amazonaws.com"
        }
      }
    }]
...
resource "aws_iam_role_policy_attachment" "vault_monitoring" {
  role       = aws_iam_role.vault_monitoring.name
  policy_arn = aws_iam_policy.vault_external_secrets.arn
}

# Vault IAM Role for Staging Namespace
resource "aws_iam_role" "vault_staging" {
  name = "${var.project_prefix}-${var.environment}-vault-staging-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
      ...
      Condition = {
        StringEquals = {
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:sub" = "system:serviceaccount:staging:staging-vault-sa"
          "${replace(var.eks_oidc_provider_arn, "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/", "")}:aud" = "sts.amazonaws.com"
        }
      }
    }]
...
resource "aws_iam_role_policy_attachment" "vault_staging" {
  role       = aws_iam_role.vault_staging.name
  policy_arn = aws_iam_policy.vault_external_secrets.arn
}
